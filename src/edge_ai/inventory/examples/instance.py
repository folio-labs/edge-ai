...
def _instance_lookups(folio_client: FolioClient) -> dict:
    lookups = dict()
    for types_info in [folio_client.contributor_types, folio_client.contrib_name_types]:
        ...
        match key:
            case "contributors":
                ...
                for contributor in payload:
                    ...
                    expanded[key].append(
                        _expand_dict(contributor, ["contributorTypeText", "name"])
                    )

            case "publication":
                expanded[key] = []
                expanded[key].extend(_expand_list(payload))
